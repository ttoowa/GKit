...
namespace WintabDN {
	...
	public class WContext {
		...
		public HCTX Open(HWND hwnd_I, bool enable_I) {
			m_hCTX = WFuncs.WTOpenA(hwnd_I, ref m_logContext, enable_I);
		...
		public bool Open() {
			...
			HWND hwnd = WMessageEvent.WindowHandle;


			m_hCTX = WFuncs.WTOpenA(hwnd, ref m_logContext, true);
		...
		public bool Close() {
			...
			bool status = WFuncs.WTClose(m_hCTX);
			m_hCTX = 0;
		...
		public bool Enable(bool enable_I) {
			...
			bool status = WFuncs.WTEnable(m_hCTX, enable_I);
		...
		public bool SetOverlapOrder(bool toTop_I) {
			...
			status = WFuncs.WTOverlap(m_hCTX, toTop_I);
