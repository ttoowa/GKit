...
namespace WintabDN {
	...
	public class WExtensions {
		...
		public static uint GetWTExtensionMask(EWTXExtensionTag tag_I) {
			uint extMask = 0;
			IntPtr buf = WMemoryUtility.AllocUnmanagedBuf(extMask);
			...
			if (extIndex != 0xFFFFFFFF) {
				...
				extMask = (uint)WMemoryUtility.MarshalUnmanagedBuf<uint>(buf, size);
			}
		...
		public static uint FindWTExtensionIndex(EWTXExtensionTag tag_I) {
			...
			IntPtr buf = WMemoryUtility.AllocUnmanagedBuf(thisTag);

			for (int loopIdx = 0, size = -1; size != 0; loopIdx++) {
				size = (int)WFuncs.WTInfoA(
					(uint)EWTICategoryIndex.WTI_EXTENSIONS + (uint)loopIdx,
					(uint)EWTIExtensionIndex.EXT_TAG, buf);

				if (size > 0) {
					thisTag = WMemoryUtility.MarshalUnmanagedBuf<uint>(buf, size);
		...
		public static bool ControlPropertyGet(
			...
			IntPtr buf = WMemoryUtility.AllocUnmanagedBuf(extProperty);
			...
			bool status = WFuncs.WTExtGet((uint)context_I, (uint)extTagIndex_I, buf);
		...
		public static bool ControlPropertySet(
			...
			retStatus = WFuncs.WTExtSet((uint)context_I, (uint)extTagIndex_I, buf);
		...
		public static bool SetDisplayProperty(
			...
			if (!WExtensions.ControlPropertyGet(
				context_I.HCtx,
				(byte)extTagIndex_I,
				(byte)tabletIndex_I,
				(byte)controlIndex_I,
				...
				ref iconFmt)) { throw new Exception("Oops - Failed ControlPropertyGet for TABLET_PROPERTY_ICON_FORMAT"); }
			...
			if ((EWTExtensionIconProperty)iconFmt != EWTExtensionIconProperty.TABLET_ICON_FMT_NONE) {
				...
				if (!WExtensions.ControlPropertyGet(
					...
					ref iconWidth)) { throw new Exception("Oops - Failed ControlPropertyGet for TABLET_PROPERTY_ICON_WIDTH"); }
				...
				if (!WExtensions.ControlPropertyGet(
					...
					ref iconHeight)) { throw new Exception("Oops - Failed ControlPropertyGet for TABLET_PROPERTY_ICON_HEIGHT"); }
		...
		private static bool SetIcon(
			...
			String imageFilePath_I) {
			try {
				if (!WExtensions.ControlPropertySetImage(
					context_I.HCtx,
					(byte)extTagIndex_I,
					(byte)tabletIndex_I,
					(byte)controlIndex_I,
					...
					imageFilePath_I)) { throw new Exception("Oops - FAILED ControlPropertySet for TABLET_PROPERTY_OVERRIDE"); }
			} catch (Exception ex) {
				throw ex;
			}
